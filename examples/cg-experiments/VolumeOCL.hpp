...
auto VolumeOCL(const Mesh &mesh, size_t iteration_mod, HPM::OpenCLHandler &hpm_ocl, Buffers& buffers, size_t work_group_size, std::string file_name, std::string kernel_name)
{
    ...
    return MeasureOCL(hpm_ocl, kernel_name, kernel, iteration_mod, work_group_size, std::tuple{GetInverseJacobian(AllCells, hpm_ocl)});

}

template <typename Mesh, typename Buffers>
auto VolumeOCLKernelTime(const Mesh &mesh, size_t iteration_mod, HPM::OpenCLHandler &hpm_ocl, Buffers& buffers, size_t work_group_size, std::string file_name, std::string kernel_name)
{
